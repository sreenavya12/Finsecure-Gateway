...
export default function TransferFunds() {
  ...
  const handleTransfer = async () => {
    ...
    // Fallback timer if modal gets stuck, though not strictly needed
  }
          ...
          <div className="w-full max-w-md bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 block">
                    Enter {recipientType === 'mobile_number' ? 'Mobile' : recipientType === 'account_number' ? 'Account Number' : 'Customer ID'}
                  </label>
                  <input
                    type="text"
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white p-4 rounded-xl font-bold transition-all shadow-[0_0_15px_rgba(59,130,246,0.2)] mt-8 disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-2"
                >
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                ...
                <div className="bg-slate-950/50 border border-slate-800 rounded-2xl p-6 space-y-4">
                  ...
                  <div className="flex justify-between items-center pb-4 border-b border-slate-800">
                    ...
                    <span className="text-white font-medium">Main Account (***1234)</span>
                  </div>
                ...
                <div className="flex gap-4 pt-4">
                  <button 
                    onClick={() => setStep(1)} 
                    disabled={loading}
                    className="w-1/3 p-4 rounded-xl bg-slate-800 text-slate-300 hover:bg-slate-700 transition-colors font-medium disabled:opacity-50"
                  >
                    Edit
                  </button>
                  <button 
                    ...
                    disabled={loading}
                    className="w-2/3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white p-4 rounded-xl font-bold transition-all shadow-[0_0_15px_rgba(59,130,246,0.3)] flex justify-center items-center disabled:opacity-50"
                  >
                    {loading ? (
            ...
            {step === 3 && (
              <div className="space-y-6 animate-in zoom-in-95 duration-500 flex flex-col items-center justify-center py-8">
                ...
                <p className="text-slate-400 text-center text-sm mb-6 max-w-[250px]">
                  ₹ {parseFloat(amount).toLocaleString()} has been securely sent to {payee}.
                </p>

                <div className="bg-slate-950 px-6 py-4 rounded-xl border border-slate-800 mb-6 flex items-center justify-between w-full">
                  <span className="text-slate-500 text-xs uppercase tracking-wider">Reference ID</span>
                  <span className="text-slate-300 font-mono text-sm">TXN{Math.floor(Math.random() * 10000000000)}</span>
                </div>

                <button
                  onClick={() => navigate("/customer-dashboard")}
                  className="w-full bg-slate-800 hover:bg-slate-700 text-white p-4 rounded-xl font-bold transition-all border border-slate-700"
                >
                  Return to Dashboard
                </button>
              </div>
